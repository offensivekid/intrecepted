...
function initDB() {
  ...
  if (!tableNames.includes('users')) {
    ...
    const adminPassword = process.env.ADMIN_PASSWORD || 'byKHvMnfwG0osUaGSvzYyg';
...
const storage = multer.diskStorage({
  ...
  filename: (req, file, cb) => {
    const name = `${Date.now()}-${crypto.randomBytes(6).toString('hex')}${path.extname(file.originalname)}`;
    cb(null, name);
  }
});
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    const allowed = /jpeg|jpg|png|gif|webp|mp4|webm|pdf|txt/;
...
// Полная защита с помощью Helmet
app.use(helmet({
  // CSP - защита от XSS
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      scriptSrc: ["'self'", "'unsafe-inline'"], // unsafe-inline нужен для inline скриптов
      ...
      baseUri: ["'self'"],
      formAction: ["'self'"],
      frameAncestors: ["'none'"], // Дополнительная защита от clickjacking
    },
  },
  // HSTS - форсит HTTPS
  hsts: {
  ...
  // Отключает X-Powered-By header (скрывает что используется Express)
  hidePoweredBy: true,
...
const corsOptions = {
  ...
  maxAge: 86400 // Кеш preflight запросов на 24 часа
};
...
// Session с максимальной защитой
app.use(session({
  ...
  name: 'sid', // Скрываем что используется Express
  resave: false,
  saveUninitialized: false,
  rolling: true, // Обновляет cookie при каждом запросе
  cookie: {
    secure: process.env.NODE_ENV === 'production', // Только HTTPS в продакшене
    httpOnly: true, // Защита от XSS - JS не может получить cookie
    maxAge: 1000 * 60 * 60 * 24, // 24 часа
    sameSite: 'strict', // Защита от CSRF - strict вместо lax
    path: '/',
    domain: undefined // Установится автоматически
  },
  proxy: true
}));
...
app.post('/api/upload', requireAuth, upload.single('file'), (req, res) => {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO media (filename, original_name, mimetype, size, thread_id, reply_id, uploaded_by, created_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      req.file.filename,
      ...
      req.body.replyId || null,
      req.session.userId,
      Date.now()
    );
    
    logSIEM('file_uploaded', 'low', req, { filename: req.file.originalname });
...
app.get('/api/profile/:username', (req, res) => {
  try {
    const user = db.prepare(`
      SELECT id, username, avatar, bio, created_at, last_login,
...
app.patch('/api/profile', requireAuth, (req, res) => {
  try {
    ...
    db.prepare('UPDATE users SET bio = ?, avatar = ? WHERE id = ?')
...
app.get('/api/search', (req, res) => {
  try {
    ...
    const threads = db.prepare(`
      ...
      FROM threads t
      JOIN users u ON t.author_id = u.id
      WHERE (t.title LIKE ? OR t.body LIKE ?) AND t.is_deleted = 0
      LIMIT 20
    `).all(query, query);
